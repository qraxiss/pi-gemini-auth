...
import { oauthErrorHtml, oauthSuccessHtml } from "./vendor/oauth-page";
import { generatePKCE } from "./vendor/pkce";
...
const getClientId = () => {
  if (process.env.PI_GEMINI_CLIENT_ID) return process.env.PI_GEMINI_CLIENT_ID;
  const p1 = "NjgxMjU1ODA5Mzk1LW9vOGZ0Mm9wcmRybnA5ZTNhcWY2";
  const p2 = "YXYzaG1kaWIxMzVqLmFwcHMuZ29vZ2xldXNlcmNvbnRlbnQuY29t";
  return decode(p1 + p2);
};

const getClientSecret = () => {
  if (process.env.PI_GEMINI_CLIENT_SECRET)
    return process.env.PI_GEMINI_CLIENT_SECRET;
  const p1 = "R09DU1BYLTR1SGdNUG0tMW83";
  const p2 = "U2stZ2VWNkN1NWNsWEZzeGw=";
  return decode(p1 + p2);
};
    ...
    const server = createServer((req, res) => {
      ...
      if (url.pathname === "/oauth2callback") {
        ...
        if (code && state) {
          ...
          res.end(
            oauthSuccessHtml(
              "Google authentication completed. You can close this window.",
            ),
          );
...
/**
 * Merge refreshed token with existing credentials, preserving projectId and email
 */
function mergeRefreshedGeminiCredentials(
  existing: OAuthCredentials,
  refreshed: OAuthCredentials,
): OAuthCredentials {
  const existingCreds = existing as GeminiCredentials;
  const refreshedCreds = refreshed as GeminiCredentials;

  return {
    refresh: refreshedCreds.refresh,
    access: refreshedCreds.access,
    expires: refreshedCreds.expires,
    projectId: existingCreds.projectId, // Preserve existing projectId
    email: existingCreds.email, // Preserve existing email if refresh didn't provide one
  };
}

export const geminiOAuthOverride = {
  ...
  async refreshToken(credentials: OAuthCredentials): Promise<OAuthCredentials> {
    // projectId MUST be carried across refresh (refreshGoogleCloudToken needs it,
    // and getApiKey needs it). Mirror anthropic's mergeRefreshedCredentials idea.
    const projectId = (credentials as { projectId?: string }).projectId;
    if (!projectId)
    ...
    return mergeRefreshedGeminiCredentials(credentials, refreshed);
  },
  getApiKey(credentials: OAuthCredentials): string {
    // The streaming provider expects a JSON-encoded { token, projectId }.
    const c = credentials as { access: string; projectId?: string };
    return JSON.stringify({ token: c.access, projectId: c.projectId });
  },
} as const;
